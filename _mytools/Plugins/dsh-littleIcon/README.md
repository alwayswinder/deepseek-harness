...
Local data lives under `$DSH_HOME/little-icon/`: `state.json` (written by the host, read by the pet), `position.json` (written by the pet), and `window.json` (the pet's report of whether DSH is on screen and whether it is in front). It is per machine and never syncs with the repository; deleting it restores the default position and state.
