...
The pet writes whether DSH is on screen into `window.json`, which selects the timer; showing or hiding the window counts as activity by itself. Each state is a four-frame loop.

Local data lives under `$DSH_HOME/little-icon/`: `state.json` (written by the host, read by the pet), `position.json` (written by the pet), and `window.json` (the pet's report of whether DSH is on screen). It is per machine and never syncs with the repository; deleting it restores the default position and state.
