...
- **Waiting on you is not work.** While a question with options or an approval is unanswered, that run counts as finished (`happy` → `idle`) instead of showing work: the loop is blocked on the person, and a pet still showing "working" makes a stalled task look busy. The host observes the `user-questions/request` and `approval/request` waterfalls — observation only, always delegating with `next()` — and leaves an agent waiting for the user out of the busy set; input queued behind that question does not count either, since the person answers first. The answer puts the pet back to work (`alert` → `working`).
...
**Errors have no expression of their own.** `agent/error` is still in the event table, but under the current rules `alert` means "a task just started". Giving failures a face again is a matter of listening for that event in the host half and feeding the state machine one more input. "Waiting for your answer" is wired up now, but as *work ended* (`happy` → `idle`) rather than as an expression of its own; a dedicated one (holding the startle frames, say) would need another state-machine input and the art to go with it.
